...
- 연습장과 펜을 준비합니다.
- 알고리즘 문제를 읽고 분석한 후
- 간단하게 테스트용으로 매우 간단한 경우부터 복잡한 경우 순서대로 생각해보면서, 연습장과 펜을 이용하여 알고리즘을 생각함
- 가능한 알고리즘이 보인다면, 구현할 알고리즘을 세부 항목으로 나누고, 문장으로 세부 항목을 나누어서 적어봄
- 코드화하기 위해, 데이터 구조 또는 사용할 변수를 정리
- 각 문장을 코드 레벨로 적음
- 변수가 코드에 따라 어떻게 변하는지를 손으로 적으면서, 임의 데이터로 코드가 정상 동작하는지를 연습장과 펜으로 검증함


#### 정렬(Sorting)이란?
``` 
 - 어떤 데이터들이 주어 졌을 때 이를 정해진 순서로 나열한 것     // 210928
...
#### 시간 복잡도 
| 명칭          | 시간 복잡도    | 최악의 경우 |완전 정렬시|
| ------------- |:-------------:| :-------------:|:-------------:|
| 버블 정렬| O(n^2) | n(n-1)/2| O(n)|
| 선택 정렬| O(n^2) | n(n-1)/2 | O(n)|
| 삽입(셔틀) 정렬| O(n^2) | n(n-1)/2 | O(n)|
| 셸 정렬 | O(n^1.25) | ||
| 퀵 정렬 | O(n log n) | ||
| 힙 정렬 | O(n log n) | ||
| 병합 정렬 | O(n log n) | ||
...
- 동적계획법(DP)
  - 상향식 접근법
  - 가장 작은 문제부터 해결한 후 각각의 결과값을 메모리에 저장하여 불러와 문제를 해결해감 
    - Memoization기법 사용 
      - Memoization(메모이제이션)이란? 프로그램 실행시 이전에 계산한 값을 저장하여, 다시 계산하지 않도록 하여 전체 실행 속도를 빠르게 하는 기술 
    - **작은 문제로 분할시 문제가 중복되어, 각 결과값 재활용함**
    -  ex) 피보나치 수열 
- 분할 정복 
  - 상위의 해답을 구하기 위해 작은 문제로 나누어서 각각 풀고 합병하면서 문제의 답을 얻는 알고리즘
  - 하양식 접근법 
  - **작은 문제로 분할시 문제는 서로 중복되지 않음**
  - ex) 병합정렬, 퀵정렬  
- 공통점
  - 큰 문제를 가장 작은 단위로 분할하여 품
- 차이점 
  - 동적 계획법
    - 부분 문제는 중복되어, Memoization 저장 후 결과값 활용됨
  - 분할 정복
    - 부분 문제는 중복되지 않음 , Memoization 사용안함 